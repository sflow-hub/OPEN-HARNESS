...
async function immutableSnapshot(db: D1Database, runId: string, fallback?: AgentProfile) {
  ...
  const snapshot = { ...fallback, effectiveModel: fallback.model.inherit ? DEFAULT_MODEL : fallback.model, workspaceRevision: 0 };
...
async function createHostedRun(db: D1Database, profile: AgentProfile, prompt: string, conversationId: string, parentRunId: string | null = null, depth = 0) {
  const runId = id(), now = stamp(), snapshot = { ...profile, effectiveModel: profile.model.inherit ? DEFAULT_MODEL : profile.model, workspaceRevision: 0 };
  await db.batch([
  ...
  try {
    ...
    if (pathname.startsWith('/v1/runner/')) {
      ...
      if (pathname === '/v1/runner/heartbeat' && request.method === 'POST') {
        ...
        for (const command of leased) { const error = 'Runner restarted after accepting this work. Completed events were preserved and the task was not replayed.'; const run = await db.prepare('SELECT id FROM runs WHERE command_id=?').bind(command.id).first<Row>(); await db.batch([db.prepare("UPDATE runner_commands SET state='failed',finished_at=?,result_json=? WHERE id=? AND state='leased'").bind(now, JSON.stringify({ error, interrupted: true }), command.id), db.prepare("UPDATE runs SET state='interrupted',error=?,updated_at=? WHERE command_id=? AND state NOT IN ('completed','failed','cancelled','interrupted')").bind(error, now, command.id), ...(run ? [db.prepare('DELETE FROM run_credentials WHERE run_id=?').bind(run.id)] : [])]); if (run) await appendRunEvent(db, String(run.id), 'run.interrupted', { error }); }
