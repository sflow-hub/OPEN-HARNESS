...
export default function AgentSettings({ agent, client, onClose, onSaved }: Props) {
  ...
  async function createPairing() { setComputerBusy(true); setComputerStatus(''); try { const result = await client.request<{ command: string; expiresAt: string }>('/v1/machines', { method: 'POST', body: JSON.stringify({ name: pairName, platform: pairPlatform }) }); setPairing(result); setComputerStatus('Pairing code created. Run this command on the computer within 10 minutes.'); } catch (err) { setComputerStatus(errorText(err)); } finally { setComputerBusy(false); } }
