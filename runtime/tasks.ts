...
export class TaskStore {
  constructor(private db: DatabaseSync) {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS task_comments (
        id TEXT PRIMARY KEY, task_id TEXT NOT NULL, body TEXT NOT NULL, created_at TEXT NOT NULL,
        FOREIGN KEY(task_id) REFERENCES tasks(id) ON DELETE CASCADE
      );
      ...
      CREATE TABLE IF NOT EXISTS task_runs (
        ...
        started_at TEXT NOT NULL, PRIMARY KEY(task_id,run_id),
        FOREIGN KEY(task_id) REFERENCES tasks(id) ON DELETE CASCADE
      );
  ...
  private board(row: Row) {
    ...
    return { id: row.id, name: row.name, archived: bool(row.archived), revision: Number(row.revision), stages, createdAt: row.created_at, updatedAt: row.updated_at };
  }
  ...
  updateBoard(boardId: string, input: Row) {
    ...
    this.db.prepare("UPDATE task_boards SET name=?,archived=?,revision=revision+1,updated_at=? WHERE id=?")
      .run(input.name === undefined ? board.name : required(input.name, "Board name", 120), archived ? 1 : 0, stamp(), boardId);
    return this.getBoard(boardId);
  }
  ...
  private task(row: Row) {
    ...
    const comments = (this.db.prepare("SELECT id,body,created_at FROM task_comments WHERE task_id=? ORDER BY created_at DESC").all(taskId) as Row[]).map(item => ({ id: item.id, body: item.body, createdAt: item.created_at }));
    ...
    const runs = (this.db.prepare("SELECT runs.*,task_runs.attempt,task_runs.started_at FROM task_runs JOIN runs ON runs.id=task_runs.run_id WHERE task_runs.task_id=? ORDER BY task_runs.attempt DESC").all(taskId) as Row[]).map(run => ({
      ...
      result: run.result, error: run.error, attempt: Number(run.attempt), startedAt: run.started_at,
    }));
    return {
  ...
  comment(taskId: string, input: Row) {
    ...
    this.db.prepare("INSERT INTO task_comments(id,task_id,body,created_at) VALUES(?,?,?,?)").run(id(), taskId, body, createdAt);
  ...
  start(taskId: string, input: Row, createRun: (input: { agentId: string; conversationId?: string; prompt: string }) => RunRow) {
    ...
    const duplicate = this.db.prepare("SELECT run_id FROM task_runs WHERE idempotency_key=?").get(key) as Row | undefined;
    ...
    const board = this.getBoard(task.boardId), progress = board.stages.find(stage => stage.category === "in_progress");
    if (!progress) throw new Error("This board needs an In progress stage.");
    const checklistText = task.checklist.map(item => `- [${item.done ? "x" : " "}] ${item.text}`).join("\n");
    const feedback = String(input.feedback || "").trim();
    const prompt = [`Task: ${task.title}`, task.description, checklistText && `Checklist:\n${checklistText}`, feedback && `Requested changes:\n${feedback}`].filter(Boolean).join("\n\n");
    let run!: RunRow;
    this.transaction(() => {
      ...
      this.db.prepare("INSERT INTO task_runs(task_id,run_id,attempt,instructions_snapshot,checklist_snapshot,idempotency_key,started_at) VALUES(?,?,?,?,?,?,?)")
        .run(taskId, run.id, attempt, task.description, JSON.stringify(task.checklist), key, stamp());
  ...
  syncRun(run: RunRow) {
    ...
    if (run.state === "completed") {
      const review = this.stage(task.boardId, "review");
