...
export class Machines {
  ...
  command(id: string) { const row = this.db.prepare('SELECT * FROM runner_commands WHERE id=?').get(id) as CommandRow | undefined; return row ? { id: row.id, machineId: row.machine_id, agentId: row.agent_id, kind: row.kind, state: row.state, result: row.result_json ? JSON.parse(row.result_json) : null } : null; }
